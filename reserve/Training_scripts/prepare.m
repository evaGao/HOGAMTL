...
% Output images' path
outputDir = strcat(inputDir, 'rawint_data/');
...
for i =  1: nImages
   ...
   % imagePatches = divideImage(image, patchSize);%对新图像进行分块
    imagePatches = getImagePatches(image, patchSize);
    saveImage(imagePatches,imageName,outputDir,patchSize);%将图像块按照一定规律保存下来
