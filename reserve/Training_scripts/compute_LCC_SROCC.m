...
function [outputImageScores, gTruthImageScores]=compute_LCC_SROCC()
...
out = fopen('/home/xiaogao/下载/NR-IQA-CNN-master1/outputs/out_lbp.txt', 'wt');
if(out == -1)
...
for i = 1 : nNets
    ...
    %outFile = fopen(strcat('/media/xiaogao/GLORIA/SCIQAD/new_outputs/8.29/new/output','.txt'),'r');
    outFile = fopen(strcat('/home/xiaogao/下载/NR-IQA-CNN-master1/outputs/outputs_lbp','.txt'),'r');
    outputPatchScores = fscanf(outFile, '%f');
    fclose(outFile);

    % Read score file for image patches into an array (ground truth)
    %gTruthFile = fopen(strcat('/media/xiaogao/GLORIA/SCIQAD/new_outputs/8.29/new/scores','.txt'),'r');
    gTruthFile = fopen(strcat('/home/xiaogao/下载/NR-IQA-CNN-master1/outputs/scores_lbp','.txt'),'r');
...
while(i <= nPatches)
    if(prev_tPatchScore ~= tPatchScores(i))
        ...
        oImageScores(outIndex) = oImageScores(outIndex);
        outIndex = outIndex + 1;
        num = 1;
